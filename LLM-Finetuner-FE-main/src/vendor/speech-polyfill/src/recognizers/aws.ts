...
declare const SpeechGrammar: any;
...
class AWSRecognizer extends CustomEventTarget {
  ...
  static create(config: configArgs): any {
  ...
  private handleSocketMessages() {
    ...
    if (connection) {
      connection.onmessage = createPipe(
        prop('data'),
        Buffer.from,
        (buffer: Buffer) => eventStreamMarshaller.unmarshall(buffer) as unknown as MessageEvent,
        ifElse(
  ...
  get grammars() {
    console.warn('`grammars` is not yet implemented in the AWS polyfill')
    return SpeechGrammar ? new SpeechGrammarList() : ([] as unknown as any)
  }
